...
class PositionalEncoding(nn.Module):
    ...
    def create_pe_table(self, d_model, max_len):
        ...
        # Create an empty tensor for the positional encoding table
        pe = torch.zeros(1, max_len, d_model)
        
        # Create a tensor of positions
        position = torch.arange(0, max_len, dtype=torch.float).unsqueeze(1)
        
        # Create a tensor of div_term to calculate wavelengths
        div_term = torch.exp(torch.arange(0, d_model, 2, dtype=torch.float) * (-math.log(10000.0) / d_model))
        
        # Fill the positional encoding table
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Only use the positional encodings for the actual sequence length
