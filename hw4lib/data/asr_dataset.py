...
class ASRDataset(Dataset):
    def __init__(
        ...
        # TODO: Take subset
        if 'subset' in self.config and self.config['subset'] < 1.0:
        ...
        # For non-test partitions, also load text files
    ...
    def __getitem__(self, idx) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        """
        # TODO: Load features
        # Convert stored numpy features to FloatTensor and normalize
        ...
        # Prepare transcripts (None in test-clean)
        if self.partition!='test-clean':
            # TODO: Get transcripts for non-test partitions
    ...
    def collate_fn(self, batch) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        feats = [feat.permute(1,0) for feat,_,_ in batch]  # (T, F)
        ...
        feat_lengths = torch.LongTensor([f.shape[1] for f,_,_ in batch])
        ...
        padded_feats = pad_sequence(feats, batch_first=True, padding_value=0.0)  # (B, T, F)
